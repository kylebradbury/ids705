...
### Create a symbolic link to the `course26` directory.
The `coursess26` directory has 1 Terabyte of shared storage space for the course. A symbolic link is a shortcut to another file system. When you first login, you will be in your home directory, create your shortcuts there.

**Command:**  
```bash
ln -s /hpc/group/coursess26 ids705
```

To verify that the symbolic link has been created correctly, you should see the `ids705` folder in the home directory pointing to the `/hpc/group/coursess26` directory by using a command such as:
```bash
ls -lrth
```
...
![](img/dcc_documentation_images/image-2-26.png)
...
- Create a personal directory under: `/hpc/group/coursess26/ids705` and name it after your NetID.  
- Be mindful of shared resources and job submissions.  
- **Questions?** Contact: paul.markakis@duke.edu.  
...
- The DCC is an open cluster built for the purpose of research and collaboration. 
- Access to the `/hpc/group/coursess26/ids705` directory is shared across all IDS705 students and staff. 
- Anything that you need to store privately should be stored in your home directory (all DCC users have 25GB of home space at `/hpc/home/<netID>`). 
