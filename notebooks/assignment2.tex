...
\begin{tcolorbox}[enhanced jigsaw, colframe=quarto-callout-note-color-frame, breakable, colback=white, rightrule=.15mm, opacitybacktitle=0.6, leftrule=.75mm, titlerule=0mm, opacityback=0, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, toprule=.15mm, colbacktitle=quarto-callout-note-color!10!white, toptitle=1mm, bottomtitle=1mm, arc=.35mm, left=2mm, bottomrule=.15mm, coltitle=black]
...
\subsection{{[}OPTIONAL{]} Bonus Exercise: Designing a performant,
low-latency classification
system}\label{optional-bonus-exercise-designing-a-performant-low-latency-classification-system}

\emph{Bonus questions can offset missed points in an assignment, but the
total score on an assignment cannot exceed 100 points}

{[}5 points{]}

For machine learning models, latency can be an important factor
depending on the application. There are generally two types of latency
to consider with a model: training time and prediction time. For many
models, you train the model once, so this could be a one-time event.
However, prediction time can be far more important for real-world
applications. If you're developing a system for a self-driving car -
your predictions need to be made very quickly to be useful so you don't
accidentally hit something. Similarly, if you're an online retailer and
are trying to make a recommendation for your customer, if it takes 2
minutes for the recommendation to load, the customer will likely have
moved on.

You're goal is to design a system that has minimal latency for each
prediction while remaining useful. You are able to adjust your KNN model
characteristics (changing the value of K, the number of features used,
the size of the training data, etc.). Of course, you also are interested
in the performance of the model. Walk through each consideration that
you factor in, showing plots of the impact varying modeling choices has
on your average latency and other key performance metrics. Describe your
process for evaluating the tradeoffs and justify your final
recommendation for the model design supported by evidence from your
analysis. Show and describe your analysis of the various factors that
are involved in making this decision and how they vary.

For this problem, use the
\href{https://archive.ics.uci.edu/dataset/2/adult}{Census Income
dataset} where the goal is to predict whether or not each individual
makes greater than \$50k per year. You will need to determine how to
best structure the problem and preprocess the data (which have not been
cleaned). Describe and justify the choices you make and be sure to plot
some sample of the data to explore your data before you jump into
modeling.
